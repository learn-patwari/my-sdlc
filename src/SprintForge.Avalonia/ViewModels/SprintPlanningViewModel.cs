...
public sealed partial class WorkItemRow : ObservableObject
{
    ...
    [ObservableProperty] private int    _durationDays;             // 0 = no bar
    ...
    public bool   HasBar      => DurationDays > 0;
    public bool   IsEpic      => Type == "Epic";
    public bool   IsSubTask   => Type == "SubTask";
    ...
    // Computed color properties (replace WPF DataTriggers in Avalonia views)
    public string BarColorHex => Status switch
    {
        "Done"        => "#22C55E",
        "In Progress" => "#3B82F6",
        _             => "#7C3AED"
    };
    public string TypeBgHex => Type switch
    {
        "Epic"    => "#4C1D95",
        "Story"   => "#2D1B69",
        "SubTask" => "#0F172A",
    ...
    public string TypeFgHex => Type switch
    {
        "Epic"    => "#C4B5FD",
        "Story"   => "#A78BFA",
        _         => "#94A3B8"
    ...
    partial void OnDurationDaysChanged(int value)
    {
        ...
        OnPropertyChanged(nameof(HasBar));
        OnPropertyChanged(nameof(EndDateLabel));
        OnPropertyChanged(nameof(DateRangeLabel));
    }
    partial void OnStatusChanged(string value)
    {
        ...
        OnPropertyChanged(nameof(StatusFgHex));
    }
    partial void OnTypeChanged(string value)
    {
        OnPropertyChanged(nameof(TypeBgHex));
        OnPropertyChanged(nameof(TypeFgHex));
        OnPropertyChanged(nameof(IsEpic));
...
public sealed partial class SprintPlanningViewModel : ObservableObject
{
    ...
    public ObservableCollection<WorkItemRow> TaskItems    { get; } = [];
    ...
    {
        int nextDay = TaskItems.Count > 0 ? TaskItems.Max(t => t.StartDay + t.DurationDays) : 0;
    ...
    [RelayCommand] private void ShiftStartLeft(WorkItemRow item)  => item.StartDay    = Math.Max(0,  item.StartDay - 1);
    [RelayCommand] private void ShiftStartRight(WorkItemRow item) => item.StartDay    = Math.Min(14, item.StartDay + 1);
    [RelayCommand] private void ExtendDuration(WorkItemRow item)  => item.DurationDays = Math.Min(15, item.DurationDays + 1);
    [RelayCommand] private void ShrinkDuration(WorkItemRow item)  => item.DurationDays = Math.Max(1,  item.DurationDays - 1);
