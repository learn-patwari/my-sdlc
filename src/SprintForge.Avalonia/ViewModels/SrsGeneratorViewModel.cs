...
public sealed partial class SrsGeneratorViewModel : ObservableObject
{
    ...
    [ObservableProperty] private string _statusMessage    = "Paste or type your requirements, then click Generate.";
    [ObservableProperty] private string _jiraProjectKey   = string.Empty;
    [ObservableProperty] private DocumentDraft? _currentDraft;
    [ObservableProperty] private bool   _canSubmitForApproval;
    ...
    {
        if (CurrentDraft is null) { StatusMessage = "Generate an SRS first."; return; }

        try
        {
            var result = await _srsService.SubmitForApprovalAsync(
                CurrentDraft, CurrentDraft.CorrelationId, Environment.UserName);
